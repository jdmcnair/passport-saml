...
SAML.prototype.validateSignature = function (fullXml, currentNode, cert) {
  console.log("inside SAML.prototype.validateSignature");
  var self = this;
  var xpathSigQuery = ".//*[local-name(.)='Signature' and " +
                      "namespace-uri(.)='http://www.w3.org/2000/09/xmldsig#']";
  var signatures = xpath(currentNode, xpathSigQuery);
  console.log("after var signatures = xpath(currentNode, xpathSigQuery);");
  ...
  console.log("after sig.loadSignature(signature);");
  // We expect each signature to contain exactly one reference to the top level of the xml we
  //   are validating, so if we see anything else, reject.
  if (sig.references.length != 1 )
    return false;
  console.log("after if (sig.references.length != 1 )");
  ...
  if (currentNode.getAttribute(idAttribute) != refId)
    return false;
  console.log("after if (currentNode.getAttribute(idAttribute) != refId)");
  ...
  if (totalReferencedNodes.length > 1)
    return false;
  console.log("before return sig.checkSignature(fullXml);");
  try {
    var result = sig.checkSignature(fullXml);
    console.log("var result = sig.checkSignature(fullXml);", result);
    return result;
  } catch (error) {
    console.error("error: ", error);
    console.error("fullXml: ", fullXml);
  }
};
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  Q.fcall(function(){
    if(self.options.validateInResponseTo){
      console.log("inside if(self.options.validateInResponseTo){");
      if (inResponseTo) {
        console.log("inside if (inResponseTo) {");
  ...
  .then(function() {
    console.log("into main validation function");
    ...
    console.log("after if (self.options.cert && self.validateSignature(xml, doc.documentElement, self.options.cert)) {");
    ...
    console.log("after multiple assertions check");

    if (assertions.length == 1) {
      console.log("assertions.length == 1");
      if (self.options.cert &&
          !validSignature &&
          !self.validateSignature(xml, assertions[0], self.options.cert)) {
        console.log("before throw");
        console.log("xml was: ", xml);
        throw new Error('Invalid signature');
      }
      console.log("before processValidlySignedAssertion");
      return self.processValidlySignedAssertion(assertions[0].toString(), inResponseTo, callback);
    }

    console.log("Assertions check done");
    ...
    console.log("encryptedAssertions check done");
        ...
        if (response) {
          ...
          if (!assertion) {
            ...
            if (status) {
              ...
              console.log("after statusCode && statusCode[0].$.Value === 'urn:oasis:names:tc:SAML:2.0:status:Responder'");
              ...
              console.log("after status code check");
            }
            throw new Error('Missing SAML assertion');
          }
          console.log("after !assertion");
        } else {
          console.log("self.options.cert && !validSignature");
          if (self.options.cert && !validSignature) {
            throw new Error('Invalid signature');
          }
  ...
  .fail(function(err) {
    console.error("Error in validatePostResponse:", JSON.stringify(err));
    callback(err);
  })
  .done();
};
